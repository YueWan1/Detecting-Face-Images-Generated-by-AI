...
parser.add_argument('--mode',type=int,default=0, help='mode of channel attention mechanism. Choose from 0,1,2')
parser.add_argument('--backbone', type=str, default='res50', help='architecture for binary classification. use res50 or mobilev2')
parser.add_argument('-d','--dir', nargs='+', type=str, default='./real-vs-fake/results_mask/LNP/test')
parser.add_argument('-m','--model_path', type=str, default='./checkpoints/mob_mode0_cha/model_epoch_best.pth')
...
if(not opt.size_only):
  if(opt.model_path is not None):
      if opt.backbone == 'res50':
          model = resnet50(num_classes=1,_mode = int(opt.mode))       
          state_dict = torch.load(opt.model_path, map_location='cpu')
          model.load_state_dict(state_dict['model'],strict=False)
      if opt.backbone == 'mobilev2':
          model = mobilenetv2(num_classes=1,_mode = int(opt.mode))
          state_dict = torch.load(opt.model_path, map_location='cpu')
          name_dict = dict([name,param] for name, param in state_dict['model'].items())
          del name_dict['fc.weight']
          del name_dict['fc.bias']
